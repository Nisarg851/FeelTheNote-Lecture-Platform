...
meetingToggleBtn.addEventListener("click", (e) => {
    ...
    if (currentElement.classList.contains("call-button")) {
        ...
        const child = currentElement.children[1];
        currentElement.removeChild(child);
        // currentElement.removeChild(currentElement.children[1]);
...
const menuToggleAnimation = {
    close: [
        {width: "100%", opacity: 1, transform: 'rotateY(0)'},
        {opacity: 0, width: "40%", transform: 'rotateY(-90deg)'}
    ],
    open: [
        {opacity: 0, width: "40%", transform: 'rotateY(-90deg)'},
        {width: "100%", opacity: 100, transform: 'rotateY(0deg)'}
    ]
};
const menuToggleAnimationTiming = {
    duration: 100,
    iterations: 1,
    fill: "forwards"
};
menuToggleBtn.addEventListener("click", (e) => {
    const menu = document.querySelector(".footer-elements-wrapper");
    if(menuOpen){
        menu.animate(menuToggleAnimation["close"], menuToggleAnimationTiming)
        ...
        menu.animate(menuToggleAnimation["open"], menuToggleAnimationTiming);
    }
    // menu.animate(menuToggleAnimation[menuOpen ? "close" : "open"], menuToggleAnimationTiming);
    // menu.style = menuOpen ? "display:none" : "display:flex";
    menuOpen = !menuOpen;
});
// copy text
